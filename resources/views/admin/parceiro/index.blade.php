    ...
    <div class="modal fade" id="modal-default">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
              ...
              {{ Form::open(['route' => 'parceiro.store', 'files' => true]) }}
                {{ csrf_field() }}
                    <div class="modal-body">
                      <div class="row">
                        <div class="form-group col-md-12">
                          {{ Form::label('nome', 'Nome') }}
                          {{ Form::text('nome', null, array('class' => 'form-control','maxlength' => '200')) }}
                        </div>
                        <div class="form-group col-md-12">
                        ...
                        <div class="form-group col-md-12">
                          ...
                          {{ Form::text('site', null, array('class' => 'form-control','maxlength' => '255')) }}
                        </div>
    ...
    <table id="datatable" class="table table-bordered">
        ...
        <tbody>
          @foreach($parceiro as $parceiros)
            <tr>
              <td>{{$parceiros->nome}}
                <td><img width="100px" src="{{ asset('uploads/parceiro/'. $parceiros->imagem) }}"></td>
                <td width="11%">
                  <span class="hint--top" aria-label="Editar Parceiro"><a href="{{ route('parceiro.edit', $parceiros->id) }}" style="border-radius: 50%" class="btn btn-warning waves-effect"> <i class="fa fa-pencil m-r-5"></i></a></span>
                  <span class="hint--top" aria-label="Deletar Parceiro"><button type="button" onclick="goswet({{$parceiros->id}}')" style="border-radius: 50%" class="btn btn-danger waves-effect"> <i class="fa fa-trash m-r-5"></i></button></span>
                </td>
            </tr>
          @endforeach
        </tbody>
        ...
        $.ajax({
          ...
          success: function(data){
            swal({
             ...
             function(){
               window.location = "{{ route('parceiro.index') }}";
             }
           );
          },
